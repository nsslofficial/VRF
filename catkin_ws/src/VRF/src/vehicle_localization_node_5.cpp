...
void Publish_Transform(ros::Publisher *pc_pub, int frame_number,  VRF::Transform tf);
...
VRF::Transform  Get_Transform();

pcl::PointCloud<pcl::PointXYZ>::Ptr NDT_Align(VRF::NDTCuda<pcl::PointXYZ, pcl::PointXYZ>& registration, const pcl::PointCloud<pcl::PointXYZ>::Ptr &source_cloud);
...
int main(int argc, char **argv) {
  ...
  if (data_mode == "usc"){
    ...
    ndt_cuda.setDistanceMode(VRF::NDTDistanceMode::D2D);
    ndt_cuda.setNeighborSearchMethod(VRF::NeighborSearchMethod::DIRECT1);
...
void Advertise_Transform(ros::NodeHandle *nh_ptr, std::string transform_topic, ros::Publisher *pub_ptr, int queue_size) {
  // Advertise topics
  *pub_ptr = nh_ptr->advertise<VRF::Transform>(transform_topic, queue_size);
