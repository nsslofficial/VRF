#include <VRF/cuda/ndt_cuda.cuh>

#include <thrust/device_vector.h>

#include <VRF/cuda/gaussian_voxelmap.cuh>
#include <VRF/cuda/covariance_regularization.cuh>
#include <VRF/cuda/find_voxel_correspondences.cuh>
#include <VRF/cuda/ndt_compute_derivatives.cuh>
...
NDTCudaCore::NDTCudaCore() {
  ...
  distance_mode = VRF::NDTDistanceMode::D2D;
  set_neighbor_search_method(VRF::NeighborSearchMethod::DIRECT7, 0.0);
}

NDTCudaCore::~NDTCudaCore() {}

void NDTCudaCore::set_distance_mode(VRF::NDTDistanceMode mode) {
...
void NDTCudaCore::set_neighbor_search_method(VRF::NeighborSearchMethod method, double radius) {
  ...
  switch (method) {
    ...
    case VRF::NeighborSearchMethod::DIRECT1:
      h_offsets.resize(1);
      h_offsets[0] = Eigen::Vector3i::Zero();
      break;

    case VRF::NeighborSearchMethod::DIRECT7:
      h_offsets.resize(7);
    ...
    case VRF::NeighborSearchMethod::DIRECT27:
    ...
    case VRF::NeighborSearchMethod::DIRECT_RADIUS:
...
void NDTCudaCore::create_source_voxelmap() {
  ...
  if (source_voxelmap || distance_mode == VRF::NDTDistanceMode::P2D) {
    return;
  }

  source_voxelmap.reset(new GaussianVoxelMap(resolution));
  source_voxelmap->create_voxelmap(*source_points);
  covariance_regularization(source_voxelmap->voxel_means, source_voxelmap->voxel_covs, VRF::RegularizationMethod::MIN_EIG);
}
...
void NDTCudaCore::create_target_voxelmap() {
  ...
  covariance_regularization(target_voxelmap->voxel_means, target_voxelmap->voxel_covs, VRF::RegularizationMethod::MIN_EIG);
}
