...
#include <VRF/Transform.h>
#include <VRF/CompressDiff.h>
...
// ///////////////// Goal \\\\\\\\\\\\\\\\\\\\\\\
// interpolate vehicle transform from previous frame to find initial guess
...
VRF::FastVGICPCuda<pcl::PointXYZ, pcl::PointXYZ> vgicp_cuda;
...
void callback_transform( const VRF::TransformConstPtr& vehicle_transform);
void callback_vehicle(boost::shared_ptr<pcl::PointCloud<pcl::PointXYZ>> vehicle_pc);
void callback_lidar(const VRF::CompressDiffConstPtr& diff_compressed);
...
Eigen::Matrix4f Get_Transform_as_matrix(const VRF::TransformConstPtr& tf);
...
int main(int argc, char **argv) {
  ...
  if (append =="t" ){
    ...
    log_file_distance.open(output_path+"/log_file_distance.txt");
  ...
  vgicp_cuda.setNearestNeighborSearchMethod(VRF::NearestNeighborMethod::GPU_RBF_KERNEL);
