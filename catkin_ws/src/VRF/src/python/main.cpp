...
VRF::NeighborSearchMethod search_method(const std::string& neighbor_search_method) {
  if(neighbor_search_method == "DIRECT1") {
    return VRF::NeighborSearchMethod::DIRECT1;
  } else if (neighbor_search_method == "DIRECT7") {
    return VRF::NeighborSearchMethod::DIRECT7;
  } else if (neighbor_search_method == "DIRECT27") {
    return VRF::NeighborSearchMethod::DIRECT27;
  } else if (neighbor_search_method == "DIRECT_RADIUS") {
    return VRF::NeighborSearchMethod::DIRECT_RADIUS;
  }
...
Eigen::Matrix4d align_points(
  ...
  std::shared_ptr<VRF::LsqRegistration<pcl::PointXYZ, pcl::PointXYZ>> reg;

  if(method == "GICP") {
    std::shared_ptr<VRF::FastGICP<pcl::PointXYZ, pcl::PointXYZ>> gicp(new VRF::FastGICP<pcl::PointXYZ, pcl::PointXYZ>);
    ...
    std::shared_ptr<VRF::FastVGICP<pcl::PointXYZ, pcl::PointXYZ>> vgicp(new VRF::FastVGICP<pcl::PointXYZ, pcl::PointXYZ>);
...
#ifdef USE_VGICP_CUDA
    std::shared_ptr<VRF::FastVGICPCuda<pcl::PointXYZ, pcl::PointXYZ>> vgicp(new VRF::FastVGICPCuda<pcl::PointXYZ, pcl::PointXYZ>);
    vgicp->setCorrespondenceRandomness(k_correspondences);
    vgicp->setNeighborSearchMethod(search_method(neighbor_search_method), neighbor_search_radius);
    vgicp->setResolution(voxel_resolution);
    reg = vgicp;
#else
    std::cerr << "error: you need to build VRF with BUILD_VGICP_CUDA=ON" << std::endl;
    return Eigen::Matrix4d::Identity();
#endif
  } else if (method == "NDT_CUDA") {
#ifdef USE_VGICP_CUDA
    std::shared_ptr<VRF::NDTCuda<pcl::PointXYZ, pcl::PointXYZ>> ndt(new VRF::NDTCuda<pcl::PointXYZ, pcl::PointXYZ>);
...
using LsqRegistration = VRF::LsqRegistration<pcl::PointXYZ, pcl::PointXYZ>;
using FastGICP = VRF::FastGICP<pcl::PointXYZ, pcl::PointXYZ>;
using FastVGICP = VRF::FastVGICP<pcl::PointXYZ, pcl::PointXYZ>;
#ifdef USE_VGICP_CUDA
using FastVGICPCuda = VRF::FastVGICPCuda<pcl::PointXYZ, pcl::PointXYZ>;
using NDTCuda = VRF::NDTCuda<pcl::PointXYZ, pcl::PointXYZ>;
#endif
